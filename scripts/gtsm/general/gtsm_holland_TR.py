...
#dest.createDimension('longitude',len(x_n))
#dest.createDimension('latitude',len(y_n))
#lon=dest.createVariable('longitude',d.flow_nodes.lon.value_in(units.deg).dtype.str,('longitude')) 
...
lat[:]=y_n
#dest.createDimension('time', None)                                                                                  # create unlimited dimension for time
...
#print('time:', time[:])
#wl=dest.createVariable('waterlevel', np.float64,('time','latitude','longitude'))
